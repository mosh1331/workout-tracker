...
export default function DiaryTab({
                                    ...
                                    <div className="p-4 space-y-3">
                                        <div>
                                            ...
                                            <p className="text-[10px] text-amber-400 uppercase tracking-widest font-bold">
                                                Profile Mode: {exDetails.metricType}
                                            </p>
                                        </div>
                                        ...
                                        <div className="bg-slate-900/50 rounded-xl p-3 border border-slate-700/50 space-y-2">
                                            ...
                                            {/* Fallback check if the progress state hasn't loaded yet */}
                                            {!workoutProgress[exDetails.id] || workoutProgress[exDetails.id].length === 0 ? (
                                                <div className="col-span-3 text-center py-4 text-xs text-slate-400 italic">
                                                    No active sets initialized for this exercise. Check your workout activation trigger.
                                                </div>
                                            ) : (
                                                            ...
                                                            {exDetails.metricType === 'WEIGHT_REPS' && (
                                                                <>
                                                                    <input
                                                                        type="number"
                                                                        step="5"
                                                                        // Allows input to be temporarily empty while typing
                                                                        value={setData.weight === 0 ? "" : setData.weight}
                                                                        onChange={(e) => {
                                                                            const val = e.target.value === "" ? 0 : parseInt(e.target.value) || 0;
                                                                            setWorkoutProgress(prev => ({
                                                                                ...prev,
                                                                                [exDetails.id]: prev[exDetails.id].map((set, idx) =>
                                                                                    idx === sIdx ? { ...set, weight: val } : set
                                                                                )
                                                                            }));
                                                                        }}
                                                                        className="w-14 bg-slate-800 border border-slate-600 rounded text-center text-xs font-bold py-1 text-white focus:outline-none focus:border-amber-500"
                                                                    />
                                                                ...
                                                                <input
                                                                    ...
                                                                    onChange={(e) => {
                                                                        ...
                                                                        setWorkoutProgress(prev => ({
                                                                            ...prev,
                                                                            [exDetails.id]: prev[exDetails.id].map((set, idx) =>
                                                                                idx === sIdx ? { ...set, reps: val } : set
                                                                            )
                                                                        }));
                                                                    }}
                                                                    className="w-16 bg-slate-800 border border-slate-600 rounded text-center text-xs font-bold py-1 text-white focus:outline-none focus:border-amber-500"
                                                                />
                                                            )}
  ...
  {/* FIX: Changed todayPlan.exercises to todayPlan.selectedExs */}
  {activeExerciseIndex < (todayPlan?.selectedExs?.length || 0) - 1 ? (
    <button
      onClick={() => setActiveExerciseIndex(p => p + 1)}
    ...
    <button
      onClick={() => { 
        ...
        alert('Workout logged completely to history record stores!'); 
      }}
