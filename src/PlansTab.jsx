    ...
    <div className="space-y-6">
      ...
      <section className="bg-slate-800 p-4 rounded-2xl border border-slate-700 shadow-xl space-y-4">
        <div>
          ...
          <p className="text-[11px] text-slate-400">Lock any previously created routine blueprint to a target tracking calendar date entry.</p>
        </div>
        <div className="space-y-3 pt-1">
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1">Target Calendar Date</label>
              <input 
                type="date" 
            ...
            <div>
              <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1">Choose Existing Blueprint</label>
              <select 
                value={schedulerForm.planId}
                onChange={(e) => setSchedulerForm({ ...schedulerForm, planId: e.target.value })}
                className="w-full bg-slate-900 border border-slate-700 rounded-xl px-3 py-2 text-xs font-semibold text-white focus:outline-none"
              >
      ...
      <section className="bg-slate-800 p-4 rounded-2xl border border-slate-700 shadow-xl space-y-4">
        <div>
          <h3 className="text-sm font-black text-white uppercase tracking-wider">Assemble Routine Blueprint</h3>
          <p className="text-[11px] text-slate-400">Group base movements and assign custom unique execution targets.</p>
        </div>

        <div className="space-y-3">
          <div>
            <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1">Routine Title</label>
            <input 
              type="text" 
          ...
          <div>
            <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1">Select Exercises & Map Target Metrics</label>
            <div className="space-y-2 max-h-60 overflow-y-auto no-scrollbar">
              {exercises.map(ex => {
                ...
                return (
                  <div key={ex.id} className="bg-slate-900/60 p-3 rounded-xl border border-slate-700 space-y-2">
                    ...
                    {isChecked && (
                      <div className="grid grid-cols-2 gap-2 bg-slate-800 p-2 rounded-lg text-center">
                        <div>
                          ...
                          <input 
                            ...
                            className="w-full bg-slate-900 rounded border border-slate-700 text-center text-xs font-bold text-white py-0.5" 
                          />
                        </div>
                        <div>
          ...
          <button 
            ...
            className="w-full bg-blue-500 text-white font-extrabold py-2.5 rounded-xl text-xs transition"
          >
