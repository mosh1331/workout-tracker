      ...
      <div className="space-y-5 animate-fadeIn">
        ...
        <div className="flex items-center gap-2 border-b border-slate-800 pb-3">
          <button 
            ...
            className="text-slate-400 hover:text-emerald-400 p-1 text-sm font-bold flex items-center gap-1"
          >
        ...
        <div className="space-y-3">
          <h3 className="text-xs font-black text-slate-400 uppercase tracking-wider px-1">Performance Log History</h3>
          
          {exerciseHistory.length > 0 ? (
            <div className="space-y-3">
              {exerciseHistory?.map((log, index) => (
                <div key={index} className="bg-slate-800 border border-slate-700 rounded-xl p-3.5 space-y-2.5 shadow-md">
                  <div className="flex justify-between items-center border-b border-slate-700/50 pb-1.5">
                    <span className="text-xs font-black text-white"><i className="fa-regular fa-calendar-check mr-1.5 text-emerald-400"></i>{log.date}</span>
                    <span className="text-[10px] font-medium bg-slate-900 px-2 py-0.5 rounded text-slate-400 border border-slate-700">{log.planName}</span>
                  </div>
                  ...
                  <div className="space-y-1">
                    {log.sets?.map((set, sIdx) => (
                      <div key={sIdx} className="grid grid-cols-3 text-center items-center bg-slate-900/40 py-1 rounded border border-slate-800/60 text-xs">
            ...
            <div className="text-center py-8 bg-slate-800/40 border border-dashed border-slate-700 rounded-xl text-xs text-slate-500">
    ...
    return (
      <div className="space-y-5 animate-fadeIn">
        <div className="flex items-center justify-between border-b border-slate-800 pb-2">
          <div>
            <h2 className="text-base font-black text-white">Catalog Exercise</h2>
            <p className="text-[11px] text-slate-400">Add a global core definition placeholder</p>
          </div>
          <button 
            onClick={() => setIsAdding(false)} 
            className="text-xs bg-slate-800 border border-slate-700 px-2.5 py-1 rounded-lg text-slate-300 font-bold"
          >
            Cancel
          </button>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <select 
              ...
              className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-xs font-bold text-white focus:outline-none focus:border-emerald-500"
            >
            ...
            {newExercise.imageUrls?.map((url, index) => (
              <div key={index} className="flex gap-2">
                <input 
                  ...
                  className="flex-1 bg-slate-800 border border-slate-700 rounded-xl px-3 py-1.5 text-xs text-slate-300 focus:outline-none focus:border-emerald-500" 
                />
  ...
  return (
    <div className="space-y-4 animate-fadeIn">
      <div className="flex justify-between items-center border-b border-slate-800 pb-2">
        <div>
          <h2 className="text-base font-black text-white">Exercise Catalog</h2>
          <p className="text-[11px] text-slate-400">Select an item to view targets and historic log trends</p>
        </div>
        <button 
        ...
        {exercises?.map((ex) => {
          ...
          return (
            <div 
              ...
              className="bg-slate-800 hover:bg-slate-750 border border-slate-700/80 rounded-xl p-3 flex items-center gap-3.5 cursor-pointer active:scale-[0.99] transition shadow-sm group"
            >
              ...
              <img 
                ...
                className="w-12 h-12 rounded-lg object-cover bg-slate-900 border border-slate-700/50 shrink-0" 
                alt="" 
              />
              
              <div className="flex-1 min-w-0">
                <h3 className="font-extrabold text-sm text-white truncate group-hover:text-emerald-400 transition">{ex.name}</h3>
                <div className="flex items-center gap-2 mt-0.5">
                  <span className="text-[9px] uppercase tracking-wider font-bold px-1.5 py-0.5 rounded bg-slate-900 text-slate-400 border border-slate-800">
